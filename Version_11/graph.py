...
# --- 4. Prompt Chains ---
decomposer_chain = None
sql_generation_chain = None
analysis_chain = None
histogram_analysis_chain = None
...
if db and llm:
    ...
    * **Reasoning:** The user specified a single, absolute date: "January 12 2024". I will use the format "on 2024-01-12". The query involves two metrics (return air temperature, supply air temperature) and two entities (AHU 005ca0, 005da0), so I must generate 2x2=4 sub-questions.
    * **JSON Output:**
        [
    ...
    **Concise, Grounded Analysis Paragraph:**
    ...
    HISTOGRAM_ANALYSIS_PROMPT_TEMPLATE = """
You are a data analyst familiar with HVAC systems. The data is a histogram of a specific metric’s values across a time window. Your task is to identify notable distribution patterns and interpret what they might indicate.
**CRITICAL RULES:**
- ONLY focus on the distribution of the 'value' column.
- DO NOT discuss time or trends — this is not time-series data.
- Be concise (2-3 sentences) and use speculative but grounded language like:
  - "This may indicate that..."
  - "A possible interpretation is..."
  - "The distribution suggests..."
- DO NOT mention metrics other than the one described in the user's query.
---
**Context: The data represents the following user query:**
{graph_question}
**Value Distribution Data (CSV format with only a 'value' column):**
{time_series_data}
---
**Concise, Grounded Distribution Analysis:**
"""
    histogram_analysis_chain = PromptTemplate(
        input_variables=["graph_question", "time_series_data"],
        template=HISTOGRAM_ANALYSIS_PROMPT_TEMPLATE
    ) | llm
...
if st.button("Generate Report") and question:
    if not db or not llm:
        st.error("Application is not initialized. Please check the sidebar for connection error details and restart.")
        st.stop()
    ...
    for idx, subq in enumerate(sub_questions):
        with st.expander(f"**Analysis for:** {subq}", expanded=True):
            try:
                ...
                st.write(f"➡️ **Fetching Data & Plotting {graph_type}...**")
                df = pd.read_sql(sql_query, db._engine)
                
                if df.empty or 'value' not in df.columns or ('timestamp' not in df.columns and graph_type != "Histogram"):
                ...
                fig_for_display = None
                if graph_type == "Line Chart":
                    fig_for_display = px.line(df, x='timestamp', y='value', title=subq, render_mode='svg')
                elif graph_type == "Scatter Plot":
                    fig_for_display = px.scatter(df, x='timestamp', y='value', title=subq)
                elif graph_type == "Histogram":
                    fig_for_display = px.histogram(df, x='value', title=subq, labels={'value': 'Value Distribution'})
                ...
                if fig_for_display:
                    ...
                    st.warning(f"Could not generate a {graph_type}.")
                    continue
                ...
                with st.spinner("AI is performing a general analysis..."):
                    ...
                    if graph_type == "Histogram":
                        analysis_text = histogram_analysis_chain.invoke(analysis_input)
                    else:
                ...
                if graph_type == "Line Chart":
                    ...
                    fig_for_export = px.scatter(df, x='timestamp', y='value')
                    fig_for_export.update_layout(title_text=wrapped_title, title_x=0.5, xaxis_title='Time', yaxis_title='Value')
                    fig_for_export.update_xaxes(tickformat='%b %d, %H:%M')
                elif graph_type == "Histogram":
                    fig_for_export = px.histogram(df, x='value', labels={'value': 'Value Distribution'})
                    fig_for_export.update_layout(title_text=wrapped_title, title_x=0.5, xaxis_title='Value', yaxis_title='Count')
